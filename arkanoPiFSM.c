...
void PulsaRaqIzq(void){
	...
	piLock (FLAGS_ARKANO_KEY);
	flagsArkanoFSM |= FLAG_RAQUETA_IZQUIERDA;
	piUnlock (FLAGS_ARKANO_KEY);
...
void PulsaRaqDer(void){
	...
	piLock (FLAGS_ARKANO_KEY);
	flagsArkanoFSM |= FLAG_RAQUETA_DERECHA;
	piUnlock (FLAGS_ARKANO_KEY);
...
int comprueba_tecla_pulsada(fsm_t* this){
	int result;
	piLock (FLAGS_ARKANO_KEY);
	// result vale 1 si en la variable flagsArkanoFSM se ha marcado FLAG_PELOTA, FLAG_RAQUETA_DERECHA o FLAG_RAQUETA_IZQUIERDA
	result = (flagsArkanoFSM & FLAG_PELOTA) || (flagsArkanoFSM & FLAG_RAQUETA_DERECHA) || (flagsArkanoFSM & FLAG_RAQUETA_IZQUIERDA);
	piUnlock (FLAGS_ARKANO_KEY);
	return result;
}
...
int comprueba_tecla_pelota (fsm_t* this) {
	int result;
	piLock (FLAGS_ARKANO_KEY);
	// result vale 1 si en la variable flagsArkanoFSM se ha marcado FLAG_PELOTA
	result = (flagsArkanoFSM & FLAG_PELOTA);
	piUnlock (FLAGS_ARKANO_KEY);
	return result;
}
...
int comprueba_tecla_raqueta_derecha (fsm_t* this) {
	int result;
	// result vale 1 si en la variable flagsArkanoFSM se ha marcado FLAG_RAQUETA_DERECHA
	piLock (FLAGS_ARKANO_KEY);
	result = (flagsArkanoFSM & FLAG_RAQUETA_DERECHA);
	piUnlock (FLAGS_ARKANO_KEY);
	return result;
}
...
int comprueba_tecla_raqueta_izquierda (fsm_t* this) {
	int result;
	// result vale 1 si en la variable flagsArkanoFSM se ha marcado FLAG_RAQUETA_IZQUIERDA
	piLock (FLAGS_ARKANO_KEY);
	result = (flagsArkanoFSM & FLAG_RAQUETA_IZQUIERDA);
	piUnlock (FLAGS_ARKANO_KEY);
	return result;
}
...
int comprueba_final_juego (fsm_t* this) {
	int result;
	piLock (FLAGS_ARKANO_KEY);
	// result vale 1 si en la variable flagsArkanoFSM se ha marcado FLAG_FINAL_JUEGO
	result = (flagsArkanoFSM & FLAG_FINAL_JUEGO);
	piUnlock (FLAGS_ARKANO_KEY);
	return result;
}
...
int comprueba_timeout_pelota (fsm_t* this) {
	...
	piLock (FLAGS_ARKANO_KEY);
	// result vale 1 si en la variable flagsArkanoFSM se ha marcado FLAG_FINAL_JUEGO
	result = (flagsArkanoFSM & FLAG_TIMEOUT_PELOTA);
	piUnlock (FLAGS_ARKANO_KEY);
	return result;
}
...
void MovimientoPelota(void) {
	...
	switch(flag_rebote) {
	    ...
	    case REBOTE_PERDIDA:
			// Pierdes
	    	piLock (FLAGS_ARKANO_KEY);
	    	flagsArkanoFSM |= FLAG_FINAL_JUEGO;
	    	piUnlock (FLAGS_ARKANO_KEY);
	        break;
	}
	ActualizaPantalla((tipo_arkanoPi*)(&(juego.arkanoPi)));
